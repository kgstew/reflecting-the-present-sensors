...
UltrasonicSensor sensors[NUM_SENSORS] = {
    {2, 3, 1, {0}, 0, 0, 0, false},    // Sensor 1: Trigger=2, Echo=3
    {4, 5, 2, {0}, 0, 0, 0, false},    // Sensor 2: Trigger=4, Echo=5
    ...
    for (uint8_t i = 0; i < NUM_SENSORS; i++) {
        ...
        if (current_time - sensor->last_reading_time >= 100) {
            ...
            if (!sensor->initialized) {
                ...
                readings_count++;
                if (readings_count >= READINGS_BUFFER_SIZE) {
                    sensor->initialized = true;
                    Serial.print("Sensor ");
                    Serial.print(sensor->sensor_id);
                    Serial.print(" initialized with average: ");
                    Serial.print(sensor->average_distance);
                    Serial.println(" cm");
                }
            }
        }
...
void checkForTriggers() {
    for (uint8_t i = 0; i < NUM_SENSORS; i++) {
        ...
        if (!sensor->initialized) continue;
        ...
        float change_percent = abs(current_distance - sensor->average_distance) / sensor->average_distance * 100;
        
        // Check if change exceeds threshold
        if (change_percent >= TRIGGER_THRESHOLD_PERCENT) {
